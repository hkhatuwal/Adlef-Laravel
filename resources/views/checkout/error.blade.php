...
<head>
    ...
    <title>{{ $title ?? 'Payment Error' }} - {{ config('app.name') }}</title>
    ...
    <div class="container mx-auto px-4 py-4">
        <div class="max-w-md mx-auto" data-aos="fade-up" data-aos-duration="500">
            ...
            <div class="text-center mb-4">
                ...
                <h1 class="text-2xl font-bold text-gray-900 mb-1">{{ $title ?? 'Payment Error' }}</h1>
                <p class="text-gray-600 text-sm">{{ $subtitle ?? 'An error occurred while processing your payment' }}</p>
            </div>
            ...
            <div class="compact-card rounded-lg p-4 mb-4" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="error-details">
                    ...
                    @if(isset($errorCode))
                    <div class="mt-2">
                        <span class="text-xs text-red-700">Error Code: </span>
                        <span class="error-code">{{ $errorCode }}</span>
                    </div>
                    @endif

                    @if(isset($suggestions) && is_array($suggestions))
                    <div class="text-xs text-red-700 space-y-1 mt-3">
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ $retryUrl }}" class="w-full retry-button text-white py-2.5 px-4 rounded-lg font-semibold text-center block text-sm">
                        <i class="fas fa-redo mr-2"></i>Try Again
                    </a>
                    @endif

                    @if(isset($cancelUrl) && $cancelUrl)
                    <a href="{{ $cancelUrl }}" class="w-full bg-gray-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-gray-700 transition-all duration-200 text-center block text-sm">
                        <i class="fas fa-arrow-left mr-2"></i>Return to Merchant
                    </a>
                    @endif

                    @if(isset($backUrl) && $backUrl)
                    <a href="{{ $backUrl }}" class="w-full bg-gray-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-gray-700 transition-all duration-200 text-center block text-sm">
