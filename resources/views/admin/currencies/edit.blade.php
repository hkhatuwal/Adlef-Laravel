...
<div class="container mx-auto px-6 py-8">
    <div class="max-w-3xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Currency</h1>
                <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Modify currency: {{ $currency->name }}</p>
            </div>
        ...
        <form action="{{ route('admin.currencies.update', $currency) }}" method="POST" enctype="multipart/form-data" 
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="type" id="type" 
                            ...
                            <option value="fiat" {{ old('type', $currency->type) === 'fiat' ? 'selected' : '' }}>Fiat Currency</option>
                            <option value="crypto" {{ old('type', $currency->type) === 'crypto' ? 'selected' : '' }}>Cryptocurrency</option>
                        </select>
                    </div>

                    <div>
                        <label for="conversion_rate" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Conversion Rate</label>
                        <input type="number" step="0.000001" name="conversion_rate" id="conversion_rate" value="{{ old('conversion_rate', $currency->conversion_rate) }}" 
                            class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700/50 dark:text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="mt-6">
                    <div class="flex items-center justify-between">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="active" id="active" value="1" {{ old('active', $currency->active) ? 'checked' : '' }}
                                class="sr-only peer">
            ...
            <div class="p-6">
                ...
                <div class="mt-1 flex items-center">
                    <span class="inline-block h-16 w-16 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-700">
                        <img id="icon-preview" src="{{ asset('storage/' . $currency->icon) }}" alt="{{ $currency->name }}" 
                    ...
                    <label class="ml-5 relative cursor-pointer">
                        ...
                        <input type="file" name="icon" id="icon" accept="image/*" class="sr-only"
