...
class TronService
{
    ...
    {
        try {
            $apiKey = config('keys.tron_node_secret');
            $baseUrl = config('keys.tron_node_url');
            ...
            ])->timeout(30)->post($baseUrl . '/api/transfer', $data);
     ...
     * @return array|null
     */
    public function fetchTransactions(string $walletAddress, int $minTimestamp): ?array
    {
        try {
            $response = Http::withHeaders([
                'accept' => 'application/json',
