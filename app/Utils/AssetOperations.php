...
//        $fee = $this->feeCalculator->calculateTransferFee($convertedAmount, $feePercentage, $minimumFee);
        ...
        $commission = $this->feeCalculator->calculateCommission($convertedAmount, $user, $to,Commission::TYPE_ASSET_TRANSFER);

        // Get exchange rate
        $rate = $this->currencyConverter->getExchangeRate($from, $to);

        // Calculate total
        $total = $this->feeCalculator->calculateTotal($convertedAmount, 0, $commission);
...
//        $fee = $this->feeCalculator->calculateTransferFee($convertedAmount, $feePercentage, $minimumFee);
        ...
        // Calculate commission
        $commission = $this->feeCalculator->calculateCommission($amount, $user, $to);
